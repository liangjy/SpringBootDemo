...
//    var h = window.outerHeight;
    ...
    if(obj_type==""||item==null){
        url='/NOCE/portal/pages_index_Index_home.action?appId=IntelligentRoadTestAnalysisV5&menuId='+IntelligentRoadTestScreenCompared.menu_id+'&perId='+IntelligentRoadTestScreenCompared.perId+'&id_path=new&isRedirect=true&appName=智能测评V5.0分屏对比'
            +"&day="+IntelligentRoadTest.day
            +"&lng="+lng
            +"&lat="+lat
            +"&zoom="+zoom
    ...
    url='/NOCE/portal/pages_index_Index_home.action?appId=IntelligentRoadTestAnalysisV5&menuId='+IntelligentRoadTestScreenCompared.menu_id+'&perId='+IntelligentRoadTestScreenCompared.perId+'&id_path=new&isRedirect=true&appName=智能测评V5.0Beta'
    +"&day="+IntelligentRoadTest.day
    +"&city="+item.city
...
//    	object_id = id.split(',')[1]+","+id.split(',')[2];//由于工单中没有地铁的工单，因此传入from_station_id和to_station_id
    ...
    if(obj_type != "弱区" && obj_type != "上行低速区" && obj_type != "下行低速区" && obj_type != "MOD3干扰区" && obj_type != "重叠覆盖区" && obj_type != "越区覆盖区"){
    	url += "&object_type="+obj_type;
    }

    if(obj_type == "上行低速区" || obj_type == "下行低速区" || obj_type == "MOD3干扰区" || obj_type == "重叠覆盖区" || obj_type != "越区覆盖区"){
        url += "&poorAreaType="+IntelligentRoadTest.currentLocation;
    }
...
//            console.log("弱区需要特殊处理");
            ...
            // IntelligentRoadTest.metroClick(messageData.item, 2,messageData.item.mr_flag);
            ...
            var metroScreenTimer = setInterval(function () {
                if(IntelligentRoadTest.allMetroDataObj){
                    if (IntelligentRoadTest.allMetroDataObj[messageData.item.mr_flag][messageData.item.line_id]) {
                        clearInterval(metroScreenTimer);
                        IntelligentRoadTest.getMetroStationDataByID(messageData.item.from_station_id,messageData.item.to_station_id,IntelligentRoadTest.day,messageData.item.city_id);
                        // IntelligentRoadTest.metroClick(messageData.item, 2,messageData.item.mr_flag);
                    }
                }
            }, 500);
            //还需要增加一个进入详情页的方法
            // IntelligentRoadTest.metroSecondVM.showMessage(messageData.item);
            /*var metroScreenThreeTimer = setInterval(function () {
                if(IntelligentRoadTest.metroAllSecondListResult){
                    clearInterval(metroScreenThreeTimer);
