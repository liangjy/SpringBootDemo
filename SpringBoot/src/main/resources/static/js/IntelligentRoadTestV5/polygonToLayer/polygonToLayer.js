...
//	console.log(itemData);
	...
	for(var i=0;i<IntelligentRoadTest.polygonToLayer.length;i++){
		if(IntelligentRoadTest.polygonToLayer[i].obj_type=='sector'|| IntelligentRoadTest.polygonToLayer[i].obj_type=='m3Sector'
            ||IntelligentRoadTest.polygonToLayer[i].obj_type=='olSector'||IntelligentRoadTest.polygonToLayer[i].obj_type=='cbSector'){//如果是扇区，需要根据基站id和扇区id进行判断
...
//	console.log(itemData);
	...
	for(var i=0;i<IntelligentRoadTest.polygonToLayer.length;i++){
		if(IntelligentRoadTest.polygonToLayer[i].obj_type=='sector'||itemData.obj_type=='m3Sector'
            ||itemData.obj_type=='olSector'||itemData.obj_type=='cbSector'){//如果是扇区，需要根据基站id和扇区id进行判断
			if(IntelligentRoadTest.polygonToLayer[i].statn_id==itemData.statn_id
					&&IntelligentRoadTest.polygonToLayer[i].cell_id==itemData.cell_id){
				deleteIndexArr = i;
			}
...
IntelligentRoadTest.updatePolygonLayerData = function IntelligentRoadTest_updatePolygonLayerData(){
	...
	for(var i=0;i<IntelligentRoadTest.polygonToLayer.length;i++){
		if(IntelligentRoadTest.polygonToLayer[i].obj_type=='sector'|| IntelligentRoadTest.polygonToLayer[i].obj_type=='m3Sector'
			||IntelligentRoadTest.polygonToLayer[i].obj_type=='olSector'||IntelligentRoadTest.polygonToLayer[i].obj_type=='cbSector'
			||IntelligentRoadTest.polygonToLayer[i].obj_type=='userComplain'){
...
//			var points = IntelligentRoadTest.polygonToLayer.points
	...
	if(IntelligentRoadTestSystemLayerV3.userComplainCompent!=null){
        IntelligentRoadTest.polygonToLayerComponents.polygonCanvasArr=newDataArr.concat(IntelligentRoadTestSystemLayerV3.userComplainCompent.polygonCanvasArr);
    }else{
