 ...
 * 根据百度地图定制map
 * 使用方法：
 * 		var CustomMap2 = CustomMap2.initMap(div, cityPermission, areaChangedCallbac)
 * 		CustomMap2.getBaiduMap();			//返回百度地图实例
 */
var CustomMap2 = {

	/**
     * 初始化地图
     * @param div	地图所在的div容器
     * @param areaChangedCallback	地图地市或者区县改变后调用此回调方法，
     * 		此回调方法有参数zoom,currCityName,currAreaName分别为地图当前级别，当前地市名，当前区县名
     *		如：function callbackDemo(zoom, currCityName, currAreaName){...};
     * @param cityPermission	当前用户的区域权限
     */
	initMap:function(div, cityPermission, areaChangedCallback){
		 ...
		 * 地图动态变更中心点
		 * 1.修改当前行政区域高亮边界
		 * 2.地图每次缩放都会调用updatePoint(false)更新中心点
		 * */
		//updateFlag  true为主动查询时变更
		function updatePoint(updateFlag){
			var point = map.getCenter();
			baiduGetLocation.getLocation(point,map, function(rs){
				//console.log(rs);
				...
				if(zoom==7||zoom==8){
					...
					regionType = "county";
					regionName = addComp.city+"-"+addComp.district;
				}
				//console.log("移动了 1："+regionName+";2:"+beforeRegionName);
				if(regionName != '' && (addComp.city == mapUtil_cityPermission || "广东省" == mapUtil_cityPermission)){
					//非广东省级别,修改当前行政区域高亮边界
					if(zoom!=7&&zoom!=8){
						...
						if(zoom>=10){
							borderCity(addComp.district,"county");
						}
					}
					//扇区
					/*if(isSectorShow && addComp.district != beforeAreaName){
		...
		function province(){
		  console.log("等级：省份");
		  cityBorderMap = {};								//清空地市名->地市边框的映射
		  //把21个地市分成21个对象
		  var cityList = getCityJson2().city;
		  //获取各地市对象
		  
		  for(var i=0;i<cityList.length;i++){
			  	//行政区域的点有多少个
			 if(cityList[i].name == mapUtil_cityPermission || "广东省" == mapUtil_cityPermission){
				for(var j = 0; j < cityList[i].boundaries.length; j++){
					//console.log(cityList[i].boundaries[j]);
					var ply = new BMap.Polygon(cityList[i].boundaries[j], {strokeWeight: 3, strokeColor: "#819fd1",fillOpacity:0.4,fillColor:""}); //建立多边形覆盖物
					//绑定鼠标进入事件
					...
					ply.addEventListener("click",function(e){
						  ...
						  //勾勒出当前地市的边框
						  baiduGetLocation.getLocation(e.point, map,function(rs){
								var addComp = rs.addressComponents;
								if(addComp.city == mapUtil_cityPermission || "广东省" == mapUtil_cityPermission){
									borderCity(addComp.city,"city");
								}
						  });
					});
					...
					allBorder.push(ply);
					cityBorderMap[diffArea.transPYtoCityName(cityList[i].cn_name)] = ply;
				}
			 }
		  }
		...
		function borderCity(city,type){
			...
			if(type=="city"){//地市
				...
				if(beforeCityName != brightCityName && areaChangedCallBackFun){
					areaChangedCallBackFun(map.getZoom(), brightCityName, "", map.getCenter());
					hasCalled = true;
				...
				//高亮区县更改
				if(beforeAreaName != brightCountyName && areaChangedCallBackFun){
					areaChangedCallBackFun(map.getZoom(), beforeCityName, brightCountyName, map.getCenter());
					hasCalled = true;
					beforeBorderType = "county";
				}
				if(isSectorShow && (beforeAreaName != brightCountyName || needToSwitchSection())){//高亮区县更改，重新画扇区
					removeBaseStationSectionCollection();
					console.log("zoom == " + map.getZoom());
					/*if(map.getZoom() < 15 && map.getZoom() >= 10){						//15级以下海量打点方式进行显示
						baseStationSectionCollectionShow(addComp.city,addComp.district);
			...
			for(var j = 0; j < count; j++){
				var ply = new BMap.Polygon(boundaries[j], {strokeWeight: 3, strokeColor: "#0000EE",fillColor:"",fillOpacity:1}); //建立多边形覆盖物
				map.addOverlay(ply);  //添加覆盖物
				allBorder.push(ply);
				if(type=="city"){//地市
			...
			if(beforeZoom>8 && map.getZoom()<=8){
				areaChangedCallBackFun(map.getZoom(), "广东省", "", map.getCenter());
				beforeBorderType = "province";
				beforeCityName = "广东省";
				beforeAreaName="";
			}else if(beforeBorderType != type && !hasCalled && map.getZoom() < beforeZoom){
			 ...
			 for(var i=0;i<cityList.length;i++){//地市
				 if(cityList[i].name == mapUtil_cityPermission || "广东省" == mapUtil_cityPermission){
					 if(cityList[i].area.length==0){
						 countyPolygon(cityList[i].boundaries,false,cityList[i].cn_name);
					 }else{
						 for(var j=0;j<cityList[i].area.length;j++){//区县
								countyPolygon(cityList[i].area[j].boundaries,true,cityList[i].area[j].name);
						 }
					 }
				 }
			 }
		...
		function countyPolygon(boundaries,iscounty,countyName){
			...
			for(var j = 0; j < count; j++){
				if(j==0){
					var ply = new BMap.Polygon(boundaries[j], {strokeWeight: 2, strokeColor: "#819fd1",fillOpacity:0.4,fillColor:""}); //建立多边形覆盖物
					//绑定鼠标进入事件
					ply.addEventListener("mouseover",function(e){
						if(map.getZoom()<10){
					...
					allBorder.push(ply);
					if(iscounty){
						cityBorderMap[countyName.substring(0, 2)] = ply;
					}
				}else{
					var ply2 = new BMap.Polygon(boundaries[j], {strokeWeight: 0.1, strokeColor: "#819fd1",fillOpacity:0.4,fillColor:"#ffffff"}); //建立多边形覆盖物
					map.addOverlay(ply2);  //添加覆盖物
					allBorder.push(ply2);
					if(iscounty){
						cityBorderMap[countyName.substring(0, 2)] = ply2;
					}
				}
			}                
		}
		...
		function baseStationSection(city,district){
			...
			var url = "pages_baidumap_BaiDuMap_baseStationSectionSql.action?city="+city+"&district="+district+"&number="+Math.random()+baseStationSectionQueryParam;
			$.get(url, function(data){
				...
				progressbar.submitSql(progressBarSqls,functionlist,reflectClass);
			});
		...
		function baseStationSectionCallback(data){
			...
			for(var i=0;i<json.length;i++){
				var sector = new BMap.Polygon(add_sector(new BMap.Point(json[i].longitude,json[i].latitude),0.002,0.002,json[i].antAzimuth), {strokeColor:"blue", strokeWeight:1, strokeOpacity:0.5,fillOpacity:0.0001});
				...
				sector.addEventListener("mouseover",function(e){
					...
					for(var a=0;a<json.length;a++){
						var flag = BMapLib.GeoUtils.isPointInPolygon(e.point , baseStationSectionList[a]);
						if(flag){
							size++;
							context += "扇区名称："+json[a].name+"<br/>扇区id："+json[a].id+"<br/>地市名称："+json[a].cityName
							+"<br/>网络类型："+json[a].nettype+"<br/>bscId："+json[a].bscId+"<br/>基站id："+json[a].baseStatnId
							+"<br/>经度："+json[a].longitude+"<br/>纬度："+json[a].latitude+"<br/>方位角："+json[a].antAzimuth+"<br/><br/>";
						}
					}
					//console.log(context);
					if(size>0){
...
//				});
		...
		function baseStationSectionCollectionShowCallback(data){
	        ...
	        //初始化PointCollection
	        for (var i = 0; i < json.length; i++) {
	        	 basePoints.push(new BMap.Point(json[i].longitude,json[i].latitude));
	        }
	        baseStationSectionCollection = new BMap.PointCollection(basePoints, options);  // 初始化PointCollection
		...
		function add_sector(centre,x,y,ant_angle)
		{
			...
			for(i=0;i<36;i++)
			{
				angle = (2* Math.PI/3 / 36) * i+(ant_angle-60)/180*Math.PI;
		 ...
		 * 返回此定制地图的百度地图实例
		 */
		CustomMap2.getCurrNetMap = function(){
			return map;
		}
		 ...
		 */
		CustomMap2.getRegionType = function(){
			return regionType;
		}
		
		/**
		 * 展现扇区	
		 * @param dataSource	网络类型：app、dpi
		 * @param timeGading	时间粒度：quarter、month、week、day、hour
		 * @param startTime		起始时间
		 * @param endTime		结束时间
		 */
		CustomMap2.showBaseStationSection = function(dataSource, timeGrading, startTime, endTime){
			removeBaseStationSectionCollection();
			console.log("zoom == " + map.getZoom());
			baseStationSectionQueryParam = "&perceptionQuery.timeGrading="+timeGrading
			+"&perceptionQuery.startTime="+startTime+"&perceptionQuery.endTime="+endTime+"&perceptionQuery.dataSource="+dataSource
			/*if(map.getZoom() < 15 && map.getZoom() >= 10){						//15级以下海量打点方式进行显示
				baseStationSectionCollectionShow(addComp.city,addComp.district);
			}else if(map.getZoom() >= 15){
				baseStationSection(addComp.city,addComp.district);
		...
		CustomMap2.removeBaseStationSection = function(){
			removeBaseStationSectionCollection();
			isSectorShow = false;
		}
		//		public 方法 end
		
		return CustomMap2;			//返回CustomMap2对象
	}
}