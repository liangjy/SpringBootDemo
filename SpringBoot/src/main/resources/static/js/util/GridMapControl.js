...
实例化：
 var gridMap = new GridMap(map, {
    ...
    opacity: 0.7,//透明度
    colorMode: 'gradient'//gradient:渐变的方式呈现颜色；range:按区间匹配颜色
});
绘制栅格图：
1.按整个地图一张图片的方式绘制：
gridMap.draw(data);
...
function GridMap(map, option) {
    ...
    if (option.divZindex) {
        this.divZindex = option.divZindex;
    }
...
GridMap.prototype = {
    ...
    //image:图片数据
    addTileToMap: function(tileCoord, image) {
    ...
    draw: function (data) {
        ...
        if (div) {
            $(div).parent().css("z-index", this.zIndex);
            $(div).css("z-index", this.divZindex);
        }
        canvas = null;
        context = null;
        data = null;
    ...
    drawGrid: function (arr, cxt, nePixel, swPixel) {
        ...
        cxt.fillRect(nw.x - nePixel.x, nw.y - nePixel.y, w, h);

        var x = nw.x - swPixel.x;
        var y = nw.y - nePixel.y;
...
//        }
    ...
    //绘制整个地图
    //data:描点数据，为二维数据，格式为[[中心经度,中心纬度,描点的值]]
    drawCircle: function (data) {
         ...
         var $this = this;
         $.each(data, function (i, item) {
        	 if($this.map.getBounds().containsPoint(new BMap.Point(item[0],item[1]))){
        		 $this.drawCircleItem(item, context);
        	 }
         ...
         this.imageLayer = new BMap.GroundOverlay(this.map.getBounds());
         this.imageLayer.setImageURL(canvas[0].toDataURL());
         this.imageLayer.OverlayType = 'CircleLayer';
         this.map.addOverlay(this.imageLayer);
    ...
    //绘制圆形
    drawCircleItem: function (arr, cxt) {

        var color = "";
        if (this.colorMode == "gradient") {
            //获取渐变颜色
            var rangValue = this.getRangeValue(arr[2]);
            color = Color.getGradientColor(this.gradientTresholds, rangValue);
        } else {
            //获取区间颜色
            color = Color.getColor(this.colorThresholds, arr[2]);
        }
        ...
        var mid = this.map.pointToPixel(new BMap.Point(mid_lng, mid_lat));//中心点
        var radius = 5;//单位为像素
        //圆
    	cxt.beginPath();
    	cxt.arc(mid.x, mid.y, radius, 0, Math.PI * 2, true); //Math.PI*2是JS计算方法，是圆
    	cxt.closePath();
    	cxt.stroke();
    	cxt.fill();
    ...
    //data:栅格数据，为二维数据，格式为[[最小经度,最小纬度,最大经度,最大纬度,栅格的值]]
    //SouthWestPoint:西南角点(即区域的左下角，最小经纬度)，NorthEastPoint:东北角点（即区域的右上角点，最大经纬度）
    drawForBounds: function (data,SouthWestPoint,NorthEastPoint) {

        if (this.colorThresholds.length == 0) { return; }

        var NorthWestPoint = new BMap.Point(SouthWestPoint.lng,NorthEastPoint.lat);//西北角，即矩形的左上角
        var SouthEastPoint = new BMap.Point(NorthEastPoint.lng,SouthWestPoint.lat);//东南角，即矩形的右下角
        var NorthWestPixel = this.map.pointToPixel(NorthWestPoint);//西北角像素位置，即矩形的左上角
        var SouthEastPixel = this.map.pointToPixel(SouthEastPoint);//东南角像素位置，即矩形的右下角
        var SouthWestPixel = this.map.pointToPixel(SouthWestPoint);
        var NorthEastPixel = this.map.pointToPixel(NorthEastPoint);
        var canvasWidth = Math.abs(NorthEastPixel.x-SouthWestPixel.x);
        var canvasHeight = Math.abs(NorthEastPixel.y-SouthWestPixel.y);
        ...
        var $this = this;
        $.each(data, function (i, item) {
            $this.drawGrid(item, context, NorthWestPixel, SouthEastPixel);
        });

        var  bounds = new BMap.Bounds(SouthWestPoint,NorthEastPoint);
        this.imageLayer = new BMap.GroundOverlay(bounds);
        this.imageLayer.setImageURL(canvas[0].toDataURL());
        this.imageLayer.OverlayType = 'GridLayer';
        this.map.addOverlay(this.imageLayer);
...
var Color = {
    ...
    getColor: function (thresholds, value) {
        var filters = $.grep(thresholds, function (item) { 
        	return Number(item.threshold) >= Number(value) 
        	})
