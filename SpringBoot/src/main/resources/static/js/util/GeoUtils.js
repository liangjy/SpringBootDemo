 ...
 (function() { 
     ...
     GeoUtils.isPointInRectForNOCE = function(point, bounds){
      ...
      * @returns {Boolean} 点在多边形内返回true,否则返回false
      */
     GeoUtils.isPointInPolygonForNOCE = function(point, polygon){
         //检查类型
//         if(!(point instanceof BMap.Point) ||
//             !(polygon instanceof BMap.Polygon)){
//             return false;
//         }
         ...
         // if(polygonBounds!=undefined){
        	//  if(!this.isPointInRectForNOCE(point, polygonBounds)){
         //         return false;
         //     }
         // }
         ...
         var pts = polygon.points;//获取多边形点
         var maxAndMinLngLat = this.getMaxAndMinLatLng(pts);//{max_lng: maxLng, max_lat:maxLat ,min_lng:minLng , min_lat:minLat};
     ...
     /**
      * 判断两个矩形是否相交
      * @param {Bounds} bounds1 矩形边界对象1
      * @param {Bounds} bounds2 矩形边界对象2
      * @returns {Boolean} 返回true,否则返回false
      */
     GeoUtils.isIntersectTwoRect = function(bounds1, bounds2){

         //检查类型是否正确
            if(!bounds1 || !(bounds1 instanceof BMap.Bounds)||!bounds2 || !(bounds2 instanceof BMap.Bounds)){
                throw "请检查传入参数值的合法性";
                return false;
            }

            if(bounds1.intersects(bounds2)==null){
                return false;
            }else{
                return true;
            }
     ...
     GeoUtils.getMaxAndMinLatLng = function (PointArray){
         var resultArr = {};
         var maxLng = null;
         var maxLat = null;
         var minLng = null;
