(function() {
  /*guoguo 百度地图API V2 离线版本 疑问请访问www.xiaoguo123.com
  * 搜索关键字 guoguo 即可查看修改的地方 
  * 修改于 2016-8-22 ，基于此日期前的最新API
  * 感谢 于忠达 提供的方法  
  * 本软件作为个人研究学习使用，由于授权导致的纠纷本人概不负责
  * 百度地图图片即瓦片文件请自行下载，或联系我索取(QQ 63659875 验证：百度地图API)
  * 2016-09-21 去除console.log,解决IE的错误
  */
  //console.log('baidumap loaded'); //IE error
	
	var nginxIp='132.96.171.33';/*zwb nginx*/ //nginx Ip
//	var nginxIp='10.17.41.16';/*zwb nginx*/ //nginx Ip
	var ak='BRBlNEMNKB7jwL2kAULKI66G';//ak值
	function aa(a) {
		throw a;
	}
	var j = void 0,
	...
	z.XX = {
		...
		main_domain_cdn: {
			...
			/*zwb nginx*/
			baidu: [nginxIp+":9993", nginxIp+":9994", nginxIp+":9995"],
	...
	function Jb(a) {
		...
		do c = Ib.indexOf(a.charAt(i++)), d = Ib.indexOf(a.charAt(i++)), f = Ib.indexOf(a.charAt(i++)), g = Ib.indexOf(a.charAt(i++)), c = c << 2 | d >> 4, d = (d & 15) << 4 | f >> 2, e = (f & 3) << 6 | g, b += String.fromCharCode(c), 64 != f && (b += String.fromCharCode(d)), 64 != g && (b += String.fromCharCode(e));
		while (i < a.length);
		return b
	}
	var N = x.lang.Zy;
	...
	(function() {
		...
		function b(a, b) {
			if (a.length === +a.length) for (var c = 0, d = a.length; c < d && b.call(j, c, a[c], a) !== q; c++);
		...
		var l = {
			send: function(a) {
				...
				0 < f.length && (b(f, function(d, e) {
					...
					e.push(a + "=" + b)
				}), e.push("d=[" + c.join(",") + "]"), g ? e.push("_st=" + g) : e.push("_t=" + +new Date), f = new Image, f.src = /*http://static.tieba.baidu.com/tb/pms/img/st.gif?*/"http://"+nginxIp+":9992/static?"/*zwb nginx*/ + e.join("&"), window["___pms_img_" + 1 * new Date] = f)
			}
		};
		a.prototype = {
			pc: function(a, b) {
...
//					ky: "http://nsclick.baidu.com/u.gif",
			...
			l && (l.an && i() < l.an) && (c.alog("cus.on", "time", function(a) {
				...
				ky: "http://"+nginxIp+":9992/static/pms/img/st.gif",/*zwb nginx*/
				page: g,
				p: "18"
			}))
		})(a, b, c)
	...
	function Qb(a, b) {
		a=a.replace("http://api.map.baidu.com/","http://"+nginxIp+":9992/");/*zwb nginx*//*增加的*/
		/*if (/^http/.test(a)) return; guoguo 这里判断一下，如果是调用外部资源就退出去好了  */
		if (b) {
	...
	z.Se(function(a) {
		...
		a.addEventListener("removeoverlay", function(a) {
			...
			else for (var d = 0, e = b.ze.length; d < e; d++) if (b.ze[d] === a) {
				b.ze.splice(d, 1);
				break
			}
		});
	...
	x.extend(Dc.prototype, {
		...
		lI: function(a) {
			...
			for (var g = 0; g < c; g++) {
				...
				if (!i.ZH && !(i.mp && !this.map.Ub() || i.iM && this.map.Ub())) {
					...
					w = [];
					for (d.LB = []; n < v; n++) for (y = l; y < e; y++) w.push([n, y]), d.LB.push({
						x: n,
						y: y
					});
		...
		zh: function(a) {
			...
			if (a instanceof db) b && a.Fm && (a.remove(), a.Fm = q);
				...
				if (a.mp) for (var b = 0, c = this.Eg.length; b < c; b++) a == this.Eg[b] && this.Eg.splice(b, 1);
				else {
					b = 0;
			...
			if (!this.map.Ub() && (a ? this.map.K.s_ = a : a = this.map.K.s_, a)) for (var c = p, c = "2" == z.ou ? [z.url.proto + z.url.domain.main_domain_cdn.other[0] + "/"] : [z.url.proto + z.url.domain.main_domain_cdn.baidu[0] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[1] + "/", z.url.proto + z.url.domain.main_domain_cdn.baidu[2] + "/"], d = 0, e; e = this.Km[d]; d++) if (e.f_ == o) {
				...
				e.getTilesUrl = function(b, d) {
					var e = b.x,
						...
						l = 1;
					this.map.dE() && (l = 2);
					l = "customimage/tile?&x=" + e + "&y=" + k + "&z=" + d + "&udt=20180211&scale=" + l;
	...
	nb.prototype.getTilesUrl = function(a, b) {
		var c = a.x,
			d = a.y,
			e = this.oc,
			c = Oc[Math.abs(c + d) % Oc.length] + "/image?grids=" + c + "_" + d + "_" + b + "&q=" + e.Wm + "&tags=" + e.eu + "&filter=" + e.filter + "&sortby=" + e.Gy + "&ak="+ak/*zwb ak*/+ "&age=" + e.sw + "&page_size=" + e.qN + "&format=" + e.A_;
	...
	Vc.getTilesUrl = function(a, b, c) {
      ...
      return tdir + "/" + b + "/" + d + "/" + a + bdmapcfg.imgext; guoguo 使用本地的瓦片 */
...
//		return (Tc[Math.abs(d + a) % Tc.length] + "?qt=tile&x=" + (d + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M") + "&z=" + b + "&styles=" + c + "&scaler=" + e + (6 == x.ea.la ? "&color_dep=32&colors=50" : "") + "&udt=20160730").replace(/-(\d+)/gi, "M$1")
		/*zwb nginx*/ /*star 普通地图瓦片*/
		var nginxUrl=(Tc[Math.abs(d + a) % Tc.length] + "?qt=tile&x=" + (d + "").replace(/-/gi, "M") + "&y=" + (a + "").replace(/-/gi, "M") + "&z=" + b + "&styles=" + c + "&scaler=" + e + (6 == x.ea.la ? "&color_dep=32&colors=50" : "") + "&udt=20160730").replace(/-(\d+)/gi, "M$1")
		nginxUrl=nginxUrl.replace("online0.map.bdimg.com", nginxIp+":9992/online0");
		nginxUrl=nginxUrl.replace("online1.map.bdimg.com", nginxIp+":9992/online1");
		nginxUrl=nginxUrl.replace("online2.map.bdimg.com", nginxIp+":9992/online2");
		nginxUrl=nginxUrl.replace("online3.map.bdimg.com", nginxIp+":9992/online3");
		nginxUrl=nginxUrl.replace("online4.map.bdimg.com", nginxIp+":9992/online4");
		return nginxUrl;
		/*zwb nginx*/ /*end*/
	};
	var La = new Rc("\u5730\u56fe", Vc, {
	...
	//	return (Sc[Math.abs(c + d) % Sc.length] + "u=x=" + c + ";y=" + d + ";z=" + b + ";v=009;type=sate&fm=46&udt=20141015").replace(/-(\d+)/gi, "M$1")
		/*zwb nginx*/
		var nginxUrl=(Sc[Math.abs(c + d) % Sc.length] + "u=x=" + c + ";y=" + d + ";z=" + b + ";v=009;type=sate&fm=46&udt=20141015").replace(/-(\d+)/gi, "M$1")
		nginxUrl=nginxUrl.replace("shangetu0.map.bdimg.com", nginxIp+":9992/shangetu0");
		nginxUrl=nginxUrl.replace("shangetu1.map.bdimg.com", nginxIp+":9992/shangetu1");
		nginxUrl=nginxUrl.replace("shangetu2.map.bdimg.com", nginxIp+":9992/shangetu2");
		nginxUrl=nginxUrl.replace("shangetu3.map.bdimg.com", nginxIp+":9992/shangetu3");
		nginxUrl=nginxUrl.replace("shangetu4.map.bdimg.com", nginxIp+":9992/shangetu4");
	...
	Yc.getTilesUrl = function(a, b) {
		...
		var nginxUrl= (Tc[Math.abs(c + d) % Tc.length] + "?qt=tile&x=" + (c + "").replace(/-/gi, "M") + "&y=" + (d + "").replace(/-/gi, "M") + "&z=" + b + "&styles=sl" + (6 == x.ea.la ? "&color_dep=32&colors=50" : "") + "&udt=20141015").replace(/-(\d+)/gi, "M$1")
		nginxUrl=nginxUrl.replace("online0.map.bdimg.com", nginxIp+":9992/online0");
		nginxUrl=nginxUrl.replace("online1.map.bdimg.com", nginxIp+":9992/online1");
		nginxUrl=nginxUrl.replace("online2.map.bdimg.com", nginxIp+":9992/online2");
		nginxUrl=nginxUrl.replace("online3.map.bdimg.com", nginxIp+":9992/online3");
	...
	var $c = {
		...
		ab: function(a, b, c, d, e) {
			...
			e || (d += "&res=api");
			d = d + ("&callback=BMap._rd._cbk" + f) + ("&ak="+ak);/*zwb ak*/
			Qb(d)
		}
	};
	window.u0 = $c;
	...
	Fe.prototype = {
		...
		$u: function(a) {
			if (!(te && 0 !== a.touches.length)) {
				...
				if (b.Wk) d = b.scale * b.op, d = Math.max(b.options.Vk, d), d = Math.min(b.options.Xp, d), b.op = d / b.scale, b.scale = d, b.x = b.by - b.by * b.op + b.x, b.y = b.dy - b.dy * b.op + b.y, b.Nb.style[ke] = "200ms", b.Nb.style[ie] = "translate(" + b.x + "px," + b.y + "px) scale(" + b.scale + ")" + Ee, b.Wk = q, b.refresh(), b.options.up && b.options.up.call(b, a);
				else {
					if (b.rh) {
