...
(function($){
 	$.extend($.fn, {
		jTree:function(options) {
			var op = $.extend({checkFn:null, selected:"selected", exp:"expandable", coll:"collapsable", firstExp:"expandable", firstColl:"collapsable", lastExp:"expandable", lastColl:"collapsable", folderExp:"folder_expandable", folderColl:"folder_collapsable", endExp:"end_expandable", endColl:"end_collapsable",file:"file",ck:"checked", unck:"unchecked"}, options);
		...
		genTree:function(options) {
			...
			return this.each(function(i){
				...
				if (tree.size()>0) {
					...
					$(">div", node).prepend("<div class='" + (op.showSub ? op.coll : op.exp) + "' id='"+(op.showSub ? op.coll : op.exp)+"_"+$(this).index()+"'></div>"+(op.ckbox ?"<div class='ckbox " + checked + "'></div>":"")+(op.icon?"<div class='"+ (op.showSub ? op.options.folderColl : op.options.folderExp) +"'></div>":""));
